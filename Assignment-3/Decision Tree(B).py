...
if __name__ == '__main__':
    ...
    for m in range(1,4):
         ...
         M = np.genfromtxt('C:/Users/Vedant/Downloads/monks_data/monks-'+str(m)+'.train', missing_values=0, skip_header=0, delimiter=',', dtype=int)
         ytrn = M[:, 0]
         Xtrn = M[:, 1:]

         # Load the test data
         M = np.genfromtxt('C:/Users/Vedant/Downloads/monks_data/monks-'+str(m)+'.test', missing_values=0, skip_header=0, delimiter=',', dtype=int)
         ytst = M[:, 0]
         Xtst = M[:, 1:]
         ...
         Train_Error_Array=[]
         for d in range(1,9):
            print("Depth of the tree is currently :" + str(d))
            ...
            test_error = compute_error(ytst, y_pred_test)

            print('Test Error = {0:4.2f}%.'.format(test_error * 100))
            
            Train_Error_Array.append(train_error)
            Test_Error_Array.append(test_error)
            Depth_Array.append(d)
        
         
         Train_Avg=sum(Train_Error_Array)/len(Train_Error_Array)
         Test_Avg=sum(Test_Error_Array)/len(Test_Error_Array)
         
         print('Training error Average={0:4.2f}%'.format(Train_Avg* 100))
         print('Testing Error Average= {0:4.2f}%'.format(Test_Avg* 100))
         
         plt.plot(Depth_Array,Train_Error_Array, label="Training Error")
         plt.plot(Depth_Array,Test_Error_Array, label="Testing Error")
         

         plt.title('monks-'+ str(m) +'Data')
